...
DATA_COMP: 'Manufacturing employment data cover the following worker categories: employees
  (with source data from the BLS Current Establishment Statistics (CES) program),
  and self-employed workers (with source data from the BLS Current Population Survey
  (CPS)).
...
REC_USE_LIM: ''
SDG_INDICATOR: Manufacturing employment as a proportion of total employment
SOURCE_TYPE: U.S. Bureau of Labor Statistics - https://www.bls.gov/lpc/
UNIT_MEASURE: The ratio itself is simply the ratio of two employment series. The two
  employment series which are used to calculate this ratio are in millions of jobs.
